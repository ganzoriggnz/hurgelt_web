...
const AddInvoiceZarlagaCopyModal = ({
  handleCancel,
  handleOk,
  open,
  data,
  ...
  useEffect(() => {
    registerform.setFieldsValue({
      type: "Зарлага",
      isPaid: "Хөдөлгөөнөөс хуулбарлан зарлага гаргав.",
      to_user: data?.joloochId,
      invoice_products: data?.prod ? data?.prod?.map((e: any) => {
        const producd = productsList.filter((a: any) =>
          a.value?.includes(e?.product_code)
        );
        return {
          product:
...
export default AddInvoiceZarlagaCopyModal;
