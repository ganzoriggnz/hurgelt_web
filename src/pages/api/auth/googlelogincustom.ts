...
import { authorizeGoogleCustom, logOutViaGoogleRevokeAxios } from "@/lib/utils";
...
export default async function handler(
  ...
  const { email, name, picture } = await authorizeGoogleCustom(idToken);
  ...
  if (email && name) {
    ...
    if (user) {
      const userupdate: any = { last_login: new Date(), avatar: picture };
